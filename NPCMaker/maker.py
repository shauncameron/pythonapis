...
def makename(length):
    ...
    for i in range(length):
        ...
        madenames.append(name)

    return ' '.join(madenames)

degree = ["Lawful", "Neutral", "Chaotic"]
...
def makealignment():
    ...
    made.append(degree[0])
    made.append(alignment[0])

    if made == ["Neutral", "Neutral"]:

        made = ["True", "Neutral"]

    return ' '.join(made) 

hair_state = ["scruffy", "plaitted", "neat", "tidy", "long", "glossy", "matte", "short", "straight", "frizzy"]

hair = ["dirt brown", "dark brown", "light brown", "jet black", "vanta black", "raven black", "light auburn", "dark auburn", "snow white", "shiny silver", "pale silver", "dirty blonde", "light blonde", "platinum blonde", "no"]

skin = ["fair", "pale", "palish", "dark", "light", "light brown", "charred", "tanned"]

depths = ["deep", "light", "dark", "hollow", "shining", "glimmering"]

colours = ["gold", "crimson red", "red", "yellow", "pink", "green", "purple", "orange", "blue", "black", "brown"]

def maketraits():

    height = f'{random.randint(50, 68)}'

    return f'{random.choice(hair_state)} {random.choice(hair)} hair,{random.choice(depths)} {random.choice(colours)} eyes,{random.choice(skin)} skin'

madenames = []

classes = ["cleric", "bard", "fighter", "barbarian", "monk", "warlock", "sorceror", "wizard", "druid", "ranger", "rogue", "artificer", "scholar", "butcher", "scout", "forager", "farmer", "historian", "professor", "chef", "traveler", "adventurer", "dungeoneer", "soldier", "hermit", "artist", "builder", "investigator", "guard", "Alchemist", "Baker", "Archer", "Prankster", "Fool", "Accountant"]

for i, classx in enumerate(classes):

    classes[i] = classx[0].upper() + classx[1:].lower()
...
def makepeople(amount):
    ...
    for i in range(amount):
        ...
        madepeople.append(
            f"""| {makename(random.randint(1, 4))} | {random.choice(["M", "F"])} | {random.choice(races)} | {random.choices(classes)[0]} | {random.choice(Politics)} | {makealignment()} | {maketraits()} |""")

    return madepeople

def makepages(amount):
    ...
    """##### NPC Table \n""" + 
    """| Name | Sex | Race | Class | Allegiance | Alignment | Appearance |\n""" + 
    """|:----:|:---:|:----:|:-----:|:----------:|:---------:|:----------:|\n""" + 
    '\n'.join(makepeople(27)) +'\n</div>'
    )
